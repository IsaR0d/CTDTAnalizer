...
class MainWindow(QMainWindow):
    ...
    def onTaskFinished(self, result):
        ...
        print(result['statsDuelo'])
        self.rem_total.setText(str(result['statsDuelo']['remate']))
        self.pas_total.setText(str(result['statsDuelo']['pase']))
        #self.par_total.setText(str(result['statsDuelo']['pared']))
        self.reg_total.setText(str(result['statsDuelo']['regate']))
        self.ent_total.setText(str(result['statsDuelo']['entrada']))
        self.blo_total.setText(str(result['statsDuelo']['bloqueo']))
        self.int_total.setText(str(result['statsDuelo']['intercepcion']))
        self.cab_total.setText(str(result['statsDuelo']['cabeceo']))
        self.vol_total.setText(str(result['statsDuelo']['volea']))
        self.balto_total.setText(str(result['statsDuelo']['bloqueoAlto']))
        self.bbajo_total.setText(str(result['statsDuelo']['bloqueoBajo']))
    ...
    def calcular(self):   
        ...
        extras = {'regate': 1+(self.reg_ex.value()+self.ex_tec.value())/100, 'remate': 1+(self.rem_ex.value()+self.ex_tec.value())/100, 'pase': 1+(self.pas_ex.value()+self.ex_tec.value())/100, 'entrada': 1+(self.ent_ex.value()+self.ex_tec.value())/100, 'bloqueo': 1+(self.blo_ex.value()+self.ex_tec.value())/100, 'intercepcion': 1+(self.int_ex.value()+self.ex_tec.value())/100, 'bajo': 1+(self.baj_ex.value()+self.ex_tec.value())/100, 'alto': 1+(self.alt_ex.value()+self.ex_tec.value())/100}
        ...
        otros = {'ts': 1+self.ts.value()/100, 'bond': 1+self.bond.value()/100, 'parametros': 1+self.parametros.value()/100, 'potencia': 1+self.potencia.value()/100, 'cabeceo': self.cab_sel.currentText(), 'volea': self.vol_sel.currentText(), 'formacion': self.for_sel.currentText(), 'lb': self.lb_sel.currentText(), 'color': self.col_sel.isChecked(), 'bb4': self.bb4_cb.isChecked()}
